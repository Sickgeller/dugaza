...
public class WebClientBaseApiClient implements BaseApiClient {
    ...
    // 생성자를 통한 명시적 초기화
    @Autowired
    public WebClientBaseApiClient(WebClient webClient, ObjectMapper objectMapper, WebClient.Builder builder, ApiConfig apiConfig) {
        ...
        if (apiConfig == null) {
            ...
            log.info("WebClient ApiConfig 주입 성공");
    ...
    public <T> List<T> callApi(URI uri, BiFunction<JsonNode, String, T> dtoCreator) {
        ...
        log.debug("WebClient API 호출 시작: {}", uri);
        ...
        try {
            ...
            log.debug("WebClient 응답 유효성 검사 결과: {}", isValid);

            if (!isValid) {
                // 로그 저장 - 실패
                log.error("WebClient API 응답이 유효하지 않음: {}", response);
            ...
            log.debug("WebClient API 응답 파싱 결과: {} 개의 항목", result.size());
            ...
            // 로그 저장 - 예외
            log.error("WebClient API 호출 중 예외 발생: {}", e.getMessage(), e);
            if (apiLogUtil != null && apiLog != null) {
                long executionTime = ChronoUnit.MILLIS.between(requestTime, LocalDateTime.now());
    ...
    public URI makeTourUri(String path, String... params) {
        log.info("[WebClient makeTourUri] apiConfig.getTour(): {}", apiConfig.getTour());
        log.info("[WebClient makeTourUri] apiConfig.getTour().getBaseUrl(): {}", apiConfig.getTour() != null ? apiConfig.getTour().getBaseUrl() : "tour is null");
        log.info("[WebClient makeTourUri] path: {}, params: {}", path, params);
        if (apiConfig.getTour() == null) {
        ...
        log.debug("WebClient Tour API 호출: baseUrl={}, path={}", apiConfig.getTour().getBaseUrl(), path);
    ...
    public URI makeTrainUri(String path, String... params) {
        log.info("[WebClient makeTrainUri] apiConfig.getTrain(): {}", apiConfig.getTrain());
        log.info("[WebClient makeTrainUri] apiConfig.getTrain().getBaseUrl(): {}", apiConfig.getTrain() != null ? apiConfig.getTrain().getBaseUrl() : "train is null");
        log.info("[WebClient makeTrainUri] path: {}, params: {}", path, params);
        URI uri = makeUri(apiConfig.getTrain().getBaseUrl(), path, params);
        log.debug("생성된 WebClient 기차 API URI: {}", uri);
        return uri;
    }

    @LogExecutionTime(category = "WebClientAPI")
    public URI makeExpressBusUri(String path, String... params) {
        log.info("[WebClient makeExpressBusUri] apiConfig.getExpressBus(): {}", apiConfig.getExpressBus());
        log.info("[WebClient makeExpressBusUri] apiConfig.getExpressBus().getBaseUrl(): {}", apiConfig.getExpressBus() != null ? apiConfig.getExpressBus().getBaseUrl() : "expressBus is null");
        log.info("[WebClient makeExpressBusUri] path: {}, params: {}", path, params);
        URI uri = makeUri(apiConfig.getExpressBus().getBaseUrl(), path, params);
        log.debug("생성된 WebClient 고속버스 API URI: {}", uri);
        return uri;
    }
