...
public class RestClientApiClient implements BaseApiClient {
    ...
    // 생성자를 통한 명시적 초기화
    @Autowired
    public RestClientApiClient(RestClient restClient, ObjectMapper objectMapper, ApiConfig apiConfig) {
        ...
        if (apiConfig == null) {
            ...
            log.info("RestClient ApiConfig 주입 성공");
    ...
    public <T> List<T> callApi(URI uri, BiFunction<JsonNode, String, T> dtoCreator) {
        ...
        try {
            // API 호출 시도 
            log.debug("RestClient 요청 시작: {}", uri);
            String response = restClient.get()
                    .uri(uri)
                    .retrieve()
                    .body(String.class);
            log.debug("RestClient 응답 수신: {}", response != null ? (response.length() > 100 ? response.substring(0, 100) + "..." : response) : "null");

            long executionTime = ChronoUnit.MILLIS.between(requestTime, LocalDateTime.now());

            // 응답 유효성 검사
            boolean isValid = validateResponse(response);
            log.debug("RestClient 응답 유효성 검사 결과: {}", isValid);

            if (!isValid) {
                // 로그 저장 - 실패
                log.error("RestClient API 응답이 유효하지 않음: {}", response);
            ...
            log.debug("RestClient API 응답 파싱 결과: {} 개의 항목", result.size());
    ...
    public URI makeTourUri(String path, String... params) {
        log.info("[RestClient makeTourUri] apiConfig.getTour(): {}", apiConfig.getTour());
        log.info("[RestClient makeTourUri] apiConfig.getTour().getBaseUrl(): {}", apiConfig.getTour() != null ? apiConfig.getTour().getBaseUrl() : "tour is null");
        log.info("[RestClient makeTourUri] path: {}, params: {}", path, params);
        if (apiConfig.getTour() == null) {
        ...
        log.debug("RestClient Tour API 호출: baseUrl={}, path={}", apiConfig.getTour().getBaseUrl(), path);
    ...
    public URI makeTrainUri(String path, String... params) {
        log.info("[RestClient makeTrainUri] apiConfig.getTrain(): {}", apiConfig.getTrain());
        log.info("[RestClient makeTrainUri] apiConfig.getTrain().getBaseUrl(): {}", apiConfig.getTrain() != null ? apiConfig.getTrain().getBaseUrl() : "train is null");
        log.info("[RestClient makeTrainUri] path: {}, params: {}", path, params);
        URI uri = makeUri(apiConfig.getTrain().getBaseUrl(), path, params);
        log.debug("생성된 RestClient 기차 API URI: {}", uri);
        return uri;
    }
    ...
    public URI makeExpressBusUri(String path, String... params) {
        log.info("[RestClient makeExpressBusUri] apiConfig.getExpressBus(): {}", apiConfig.getExpressBus());
        log.info("[RestClient makeExpressBusUri] apiConfig.getExpressBus().getBaseUrl(): {}", apiConfig.getExpressBus() != null ? apiConfig.getExpressBus().getBaseUrl() : "expressBus is null");
        log.info("[RestClient makeExpressBusUri] path: {}, params: {}", path, params);
        URI uri = makeUri(apiConfig.getExpressBus().getBaseUrl(), path, params);
        log.debug("생성된 RestClient 고속버스 API URI: {}", uri);
        return uri;
    }
