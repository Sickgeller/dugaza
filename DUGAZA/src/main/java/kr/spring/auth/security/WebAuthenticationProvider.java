...
public class WebAuthenticationProvider implements AuthenticationProvider {
    ...
    public Authentication authenticate(Authentication authentication) throws AuthenticationException {
        ...
        if (!passwordEncoder.matches(password, userDetails.getPassword())) {
            log.warn("웹 인증 실패 - 잘못된 비밀번호: {}", username);
            throw new BadCredentialsException("아이디 또는 비밀번호가 올바르지 않습니다.");
        }
        
        // 계정 상태 검증
        if (!userDetails.isEnabled()) {
            log.warn("웹 인증 실패 - 비활성화된 계정: {}", username);
            throw new BadCredentialsException("비활성화된 계정입니다.");
        }
        
        if (!userDetails.isAccountNonLocked()) {
            log.warn("웹 인증 실패 - 잠긴 계정: {}", username);
            throw new BadCredentialsException("잠긴 계정입니다.");
        }
        
        log.info("웹 인증 성공: {}", username);
